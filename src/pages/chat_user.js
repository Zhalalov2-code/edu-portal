...
const ChatUsers = () => {
    ...
    const getAllGroupChat = useCallback(async () => {
        try {
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                console.log("✅ Групповые чаты успешно получены:", filteredGroupChats);
            }
        } catch (error) {
    ...
    useEffect(() => {
        ...
        syncChat()
    }, [selectedChatCombined, getMessages, readMessage, user.id]);
                                            ...
                                            return (
                                                <div key={chat.id_chat} onClick={() => setSelectedChatId(chat.id_chat)}
                                                    ...
                                                    <div className="chat-user-meta">
                                                        <span
                                                            className="chat-user-title">{chat.group_name || "Группа"}</span>
                                                    </div>
                                                </div>);
                                        }
