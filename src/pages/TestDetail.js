...
const TestDetail = () => {
      ...
      try {
        ...
        if (res.status === 200) {
          ...
          if (Array.isArray(data)) {
            ...
            if (!found)
              found = data.find((d) => d && typeof d === "object") || null;
            data = found;
          }
          ...
          if (data && mounted) {
            ...
            const ca =
              data.correct_answer ?? data.correctAnswer ?? data.answer ?? null;
            if (typeof ca === "number") {
  ...
  useEffect(() => {
    ...
    const loadProgress = async () => {
      try {
        const response = await axios.get(API_LESSON_PROGRESS_URL, {
  ...
  const canTake = () => {
    if (user && user.role && user.role.toLowerCase().includes("teacher"))
      return true;
    if (test && test.lesson_id)
      return completedLessons.has(String(test.lesson_id));
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const storageKey =
        user?.id != null ? `completedTests_${user.id}` : "completedTests_guest";
        ...
        <div className="card-body">
          ...
          <div className="td-actions">
            ...
            <button
              ...
              onClick={() => navigate(`/lessons/${test.lesson_id}`)}
              disabled={!test.lesson_id}
            >
              Открыть урок
            </button>
          ...
          {result && (
            <div className="td-result">
              <strong>Результат:</strong> {result.ok ? "Пройден" : "Не пройден"}{" "}
              — {result.score}%
            </div>
          )}
        </div>
